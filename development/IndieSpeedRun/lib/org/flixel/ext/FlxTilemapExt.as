...
{	
	...
	{		
		...
		protected var SLOPE_RIGHT_FLOORS:Array = []; 
		//slopes that look like  \   and are floors
		protected var SLOPE_LEFT_FLOORS:Array = []; 
		//slopes that look like  \   and are ceilings
		//protected var SLOPE_LEFT_CEILINGS:Array = []; 
		//slopes that look like  /   and are ceilings
		//protected var SLOPE_RIGHT_CEILINGS:Array = []; 	
		//
		protected var SLOPES:Array = [];
		...
		public function updateSlopes():void 
		{
			var id:int 
			for each (id in SLOPE_RIGHT_FLOORS) setTileProperties(id, RIGHT | FLOOR, solveCollisionSlopeFloorLeft);
			for each (id in SLOPE_LEFT_FLOORS) setTileProperties(id, LEFT | FLOOR, solveCollisionSlopeFloorRight);
		}
			...
			while(row < selectionHeight)
			{
				...
				while(column < selectionWidth)
				{
					...
					if(tile && tile.allowCollisions)
					{
						...
						if (overlapFound 
						|| (!overlapFound && (isSlope(tile.index))))
						{
					...
					else if(tile && (tile.callback != null) && ((tile.filter == null) || (Object is tile.filter)))
					{
						tile.mapIndex = rowStart+column;
						tile.callback(tile,Object);
					}
		...
		private function isSlope(index:uint):Boolean 
		{
			if (SLOPES.indexOf(index) != -1) return true;
			return false;
		}
		...
		//EXPOSED Tilewidth and Tileheight
