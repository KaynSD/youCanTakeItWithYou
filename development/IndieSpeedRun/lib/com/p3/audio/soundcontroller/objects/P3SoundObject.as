package com.p3.audio.soundcontroller.objects
{
	...
	public class P3SoundObject extends EventDispatcher implements IP3SoundObject
	{
		...
		private var delayTimeoutID:int = -1;
		...
		{
			super(this);
			if ($class) _soundSource = new $class
		}
		...
		public function play ($force_restart:Boolean = false):SoundChannel
		{
			...
			{
				if (!_fadeInTimer) _channel = soundSource.play(position, 1, new SoundTransform(_volume,_pan));
				else _channel = soundSource.play(position, 1, new SoundTransform(0,_pan));
			}
			else 
			{
				_pausePosition = 0;
		...
		private function callOnComplete():void 
		{
			...
			else 
			{
				...
				else
				{
					...
					trace("P3SoundObject WARNING: onComplete function can't accept the number of parameters you want to parse.");
		...
		{
			if (_isPlaying) trace("P3SoundController - WARNING " + " destroying sounds whilst playing my have odd consequences");
