			...
			{	
				...
				if (ChatTools.testConditions(node))
				{
					if (!node.isGroup)
					{
						...
						{
							trace(this + " Final dialogue node. No link nodes found or conditions met. FINAL_DIALOGUE.");
					...
					{					
						...
						{
							trace(this + " No link nodes found or conditions met. FINISHED.");
				...
				{
					trace(this + " The Node that is meant to be displayed failed to pass its conditions. THis should not happen "
					+ "and is either an error in the Dialogue Tree in chatmapper or you are doing someting that The ChatMapper api cant handle.", node);
