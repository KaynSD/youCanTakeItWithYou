package com.p3.audio.soundcontroller 
{
	...
	public class P3SoundController extends EventDispatcher
	{
		...
		private var _autoFlushCache		:Boolean = true;
		 ...
		 * 	 <li><b> manualDestroy : Number</b>	If true the sound won't call destroy when it has finished. Call it yourself to remove the sound object from memory</li>
		 ...
		 * @param	$class the class of the sound you want to play
		 ...
		 */
		public function playSound ($class:Class, $vars:Object = null):IP3SoundObject
		{
			var instance:*
			...
			if (soundGroup && soundGroup.isMaxSounds) return null;
			
			sndObj.soundSource = instance;
			sndObj.key = getQualifiedClassName($class);
		 ...
		 * @return a referance to the IP3SoundObject you created so you can manually mess with the sound, like changing volume
		 */
		public function playRandomSound ($classes:Array, $vars:Object):IP3SoundObject
		{
		 ...
		 * @param	$class Class of the music you want to play
		 * @param	$vars object containing custom initilize parameters like volume,fadeIn,fade_out,type_cast,group,loops,delay,startTime ect ect...
		 * @param	$crossfade set to false if you want to disable auto crossfading for whatever reason (like multiple music layers).
		 * @return a referance to the IP3SoundObject you created so you can manually mess with the sound, like changing volume
		 */
		public function playMusic ($class:Class, $vars:Object = null, $crossfade:Boolean = true, $forceRestart:Boolean = false):IP3SoundObject
		{
			$vars = ($vars != null) ? $vars : { };
		...
		public function stopSound($class:Class, $fadeOut:Number = 0):void
		{
			...
			for each (var group:P3SoundGroup in _groupList.list)
			{
				if (group)
				{
					...
					{
						if (sound) sound.fadeOut($fadeOut);
					}
				}
			}
		}
		 ...
		 * @param	$groupKey unique identifier for the group you want to mute.
		 */
		public function unmuteAllSound ($groupKey:String = null):void
		{
			var targetGroups:Vector.<P3SoundGroup> = getSpecifiedGroups($groupKey);
			if (!$groupKey) 
			{
