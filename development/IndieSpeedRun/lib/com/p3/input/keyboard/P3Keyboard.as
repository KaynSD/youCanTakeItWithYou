package com.p3.input.keyboard
{
	...
	/**
	 * Modification of Adam Atomic keyboard handler class from Flixel.
	 * init with a valid stage referance.
	 * Usage syntax
	 * On pressed - keys.justPressed("SPACE")
	...
	public class P3Keyboard
	{
		...
		internal static var _inst:P3Keyboard;
		
		internal var _cheats:P3KeyboardCheats;
		...
		/**
		 * Stage intilization code to set the handler up since you don't have Flixel to do it for you.
		 */
		public function init($stage:Stage):void
		{
			...
			_stage.addEventListener(Event.ENTER_FRAME, handleUpdate);
			_stage.addEventListener(Event.DEACTIVATE, onLostFocus);
		}
		...
		/**
		 * Event handler for update (handles justPressed and justReleased).
		 * @param	e
		 */
		private function handleUpdate(e:Event):void 
		{
			update();
			if (_cheats)_cheats.update();
		}
		...
		public function destroy():void
		{
			if (_stage)
			{
				...
				_stage.removeEventListener(Event.ENTER_FRAME, handleUpdate);
			}
			_lookup = null;
			_map = null;
		}
		
		public static function get inst ():P3Keyboard
		{
			if (!_inst)
			{
				_inst = new P3Keyboard ();
			}
			return _inst;
		}
		
		public function get cheats ():P3KeyboardCheats
		{
			if (!_cheats)
			{
				_cheats = new P3KeyboardCheats ();
			}
			return _cheats;
		}
	}
