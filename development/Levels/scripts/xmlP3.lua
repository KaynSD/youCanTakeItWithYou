...
isUseAssetsDir = as3.tolua(isUseAssetDir);
...
function exportObjectXML (spriteLayer, layerFileName)
	...
	spriteText = spriteText..tab3.."<SCALE x=\"%scalex%\" y=\"%scaley%\" />\n"
...
function exportLinkXML ()
	returnText= "";
	linkText = tab3.."<LINK link_from=\"%linkfromid%\" link_to=\"%linktoid%\" link_from_str=\"%getlinkfromstr%\" link_to_str=\"%getlinktostr%\"/>\n"
...
for groupIndex = 0,groupCount do
	...
	fileText = fileText..exportLinkXML();
	fileText = fileText..tab1.."</LINKS>\n"
	...
	else DAME.WriteFile(exportDir.."/"..levelName..".xml", fileText )
	end
...
if (useIndex) then
	...
	DAME.WriteFile(exportDir.."/"..levelPre..".xml", masterFileText )
	else
	assetText = tab1.."<ASSETS>\n"
	assetText = assetText..tab2..exportClassResourcePathsXML();
	assetText = assetText..tab1.."</ASSETS>\n";
	DAME.WriteFile(exportDir.."/../".."assets.xml", assetText )
end
